...
from scripts.build_objaverse_rag_dataset import (
    REFERENCE_POSES,
    TARGET_ORDER,
    TARGET_POSES,
    build_dataset,
...
from zero123plus.rag_utils import load_reference_images, references_to_slot_weights
...
class ObjaverseRAGDatasetBuilderTest(unittest.TestCase):
    ...
    def test_config_points_to_objaverse_dataset_root(self):
        config_path = Path("configs/zero123plus-rag-adapter-objaverse-wide-1000.yaml")
        text = config_path.read_text(encoding="utf-8")

        self.assertIn("root_dir: data/rag_zero123plus_objaverse_toys", text)
        self.assertIn("manifest_fname: train.jsonl", text)
        self.assertIn("manifest_fname: val.jsonl", text)
        self.assertIn('rag_slot_weight_mode: "wide"', text)
    ...
    def test_medium_500_config_settings(self):
        ...
        config = OmegaConf.load("configs/zero123plus-rag-adapter-objaverse-wide-500obj-500steps-val.yaml")

        self.assertEqual(config.data.params.train.params.root_dir, "data/rag_zero123plus_objaverse_toys_500")
        self.assertEqual(config.data.params.validation.params.root_dir, "data/rag_zero123plus_objaverse_toys_500")
        ...
        self.assertEqual(config.model.params.rag_validation_num_inference_steps, 15)
        self.assertEqual(config.model.params.rag_debug_metrics_interval, 25)
        self.assertEqual(config.model.params.rag_slot_weight_mode, "wide")

    def test_medium_500_config_2500_steps_with_light_validation_logging(self):
        from omegaconf import OmegaConf

        config = OmegaConf.load("configs/zero123plus-rag-adapter-objaverse-wide-500obj-2500steps-val-earlystop.yaml")
    ...
    def test_existing_configs_do_not_point_to_medium_dataset(self):
        for path in [
            Path("configs/zero123plus-rag-adapter-100.yaml"),
            Path("configs/zero123plus-rag-adapter-tiny.yaml"),
            Path("configs/zero123plus-rag-adapter-objaverse-wide-1000.yaml"),
        ]:
            text = path.read_text(encoding="utf-8")
            self.assertNotIn("data/rag_zero123plus_objaverse_toys_500", text)
            self.assertNotIn("zero123plus-rag-adapter-objaverse-wide-500obj-500steps-val", text)
