...
from zero123plus.rag_adapter import (
    RAGReferenceAdapter,
...
class RagAdapterTest(unittest.TestCase):
    ...
    def test_adapter_appends_seven_tokens(self):
        adapter = RAGReferenceAdapter(embed_dim=8)
    ...
    def test_spatial_training_reuses_condition_noise_and_zero_rag_is_identity(self):
        calls = []
        ...
        model = SimpleNamespace(
            ...
            rag_token_scale=0.0,
            rag_global_scale=0.0,
            rag_debug_dump=False,
            global_rank=0,
            _rag_spatial_train_debug_printed=False,
            forward_unet=fake_forward_unet,
            _capture_rag_prediction_metrics=lambda *args: None,
        )
    ...
    def test_validation_rag_status_is_false_for_only_padded_refs(self):
        model = SimpleNamespace(enable_rag_adapter=True, rag_adapter=object())
        batch = {
            "ref_imgs": torch.zeros(1, 2, 3, 8, 8),
            "ref_valid_mask": torch.zeros(1, 2),
        }
        kwargs, uses_rag = MVDiffusion._build_validation_rag_kwargs(model, batch, 0)
        self.assertFalse(uses_rag)
        self.assertEqual(kwargs, {})

    def test_validation_rag_kwargs_include_references_and_slot_weights(self):
        model = SimpleNamespace(
            enable_rag_adapter=True,
            rag_adapter=object(),
            rag_token_scale=0.1,
            rag_global_reference_token_scale=0.1,
        ...
        self.assertTrue(uses_rag)
        self.assertEqual(len(kwargs["rag_reference_images"]), 1)
        self.assertEqual(kwargs["rag_ref_view_ids"], [1])
        self.assertEqual(kwargs["rag_ref_valid_mask"], [1.0])
        self.assertTrue(torch.allclose(
            torch.tensor(kwargs["rag_ref_slot_weights"]),
            torch.tensor([[0.05, 1.0, 0.35, 0.05, 0.35, 0.05]]),
        ))
        self.assertTrue(kwargs["rag_spatial_gating"])
    ...
    def test_controlnet_uses_separate_base_and_ref_conditioning(self):
        ...
        module(
            ...
            cross_attention_kwargs={
                ...
                "rag_spatial_mask": torch.ones(1, 1, 2, 2),
                "rag_base_encoder_hidden_states": base_states,
            },
        )
    ...
    def test_periodic_rag_metrics_are_written_as_jsonl(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            MVDiffusion._write_rag_debug_metrics(model, {
                "step": 20,
                "rag_delta_mean": 0.125,
                "ref_proj_grad_norm": 0.5,
            })
            rows = [
                json.loads(line)
                for line in (Path(tmpdir) / "rag_debug_metrics.jsonl").read_text(encoding="utf-8").splitlines()
            ]
            self.assertEqual(rows[0]["step"], 20)
            self.assertEqual(rows[0]["rag_delta_mean"], 0.125)

    def test_tensor_metrics_capture_deltas_and_adapter_grad_norms(self):
        model = SimpleNamespace(
            _should_collect_rag_metrics=lambda: True,
            _rag_forward_debug_metrics=None,
        )
        base = torch.zeros(1, 2, 2, 2)
        ref = torch.ones_like(base)
        final = torch.full_like(base, 0.25)
        MVDiffusion._capture_rag_prediction_metrics(model, base, ref, final)
        self.assertEqual(model._rag_forward_debug_metrics["rag_delta_mean"], 1.0)
        self.assertEqual(model._rag_forward_debug_metrics["final_delta_mean"], 0.25)

        adapter = RAGReferenceAdapter(embed_dim=4)
        output = adapter(
            torch.randn(1, 1, 4),
    ...
    def test_post_train_smoke_writes_ablation_outputs_and_report(self):
        class FakePipeline:
            ...
            def __call__(self, image, **kwargs):
                refs = kwargs.get("rag_reference_images") or []
                slot_weights = kwargs.get("rag_ref_slot_weights") or []
                value = 0
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            torch.save({
                "state_dict": {
                    f"rag_adapter.{name}": tensor.detach().clone()
            ...
            model = SimpleNamespace(
                ...
                rag_adapter=adapter,
                rag_smoke_test_cond_image=str(cond_path),
                rag_smoke_test_ref_dir=str(refs),
                rag_smoke_test_metadata=str(metadata_path),
                rag_smoke_test_num_inference_steps=2,
                rag_smoke_test_seed=1,
                rag_smoke_test_include_strong_scale=True,
                rag_smoke_test_strong_scale_multiplier=3.0,
                rag_smoke_test_difference_threshold=1e-6,
                rag_token_scale=0.1,
            ...
            report = run_post_train_rag_smoke_test(model, str(checkpoint_path))

            smoke_dir = Path(model.logdir) / "rag_smoke_test"
            ...
            self.assertTrue((smoke_dir / "shuffled_vs_correct_diff_heatmap.png").exists())
