...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Build an object-level Zero123++ reference adapter dataset from Objaverse-style 3D assets."
    )
    parser.add_argument("--output_root", default="data/reference_zero123plus_objaverse_toys")
    ...
    parser.add_argument(
        "--seed_dataset_root",
        default=None,
        help="Optional existing rendered reference dataset to copy into the output before rendering new assets.",
    )
...
def download_objaverse_assets(args) -> Path:
    ...
    print(f"[objaverse-reference] loading Objaverse annotations into cache {os.environ['OBJAVERSE_HOME']}")
    ...
    print(f"[objaverse-reference] downloading {len(uids)} Objaverse assets")
    ...
    print(f"[objaverse-reference] wrote downloaded metadata: {metadata_path}")
...
def render_candidate(candidate: Candidate, object_dir: Path, args) -> None:
    ...
    if bpy is None:
        raise RuntimeError(
            ...
            f"{args.blender_path} --background --python scripts/build_objaverse_reference_dataset.py -- --source_dir <models> "
...
def normalize_scene(target_size: float) -> Dict:
    ...
    root = bpy.data.objects.new("ReferenceDatasetRoot", None)
...
def print_summary(report: Dict) -> None:
    ...
    print("[objaverse-reference] dataset build summary")
    ...
    print("[objaverse-reference] suggested training command:")
...
def main(argv=None):
    ...
    if args.download_objaverse:
        try:
            ...
            print(f"[objaverse-reference:error] {exc}")
            return 2
    ...
    try:
        ...
        print(f"[objaverse-reference:error] {exc}")
        print("[objaverse-reference:setup] Install/use Blender for rendering local 3D assets, and provide --source_dir or --metadata_path.")
        if args.allow_empty:
            return 0
        return 2
