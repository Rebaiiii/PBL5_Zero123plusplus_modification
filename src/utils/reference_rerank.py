...
def generate_zero123plus_candidate(
    ...
    **pipeline_kwargs,
    ...
    Supplied reference images are not passed into this function and are not
    injected into Zero123++ conditioning. They are used only after this call to
    rerank complete generated sheets.
    """
...
def compute_reference_score(
    ...
    for ref_embed, reference in zip(ref_embeds, references):
        ...
        reference_scores.append(float(similarities.max().item()) * view_weight)
    reference_score = float(np.mean(reference_scores)) if reference_scores else 0.0

    inconsistency_penalty = _pairwise_mean_similarity(tile_embeds)
    total_score = input_preservation_score + reference_weight * reference_score - inconsistency_penalty
...
def select_best_candidate(
    ...
    for seed, sheet in candidates:
        ...
        score = compute_reference_score(input_embed, tile_embeds, ref_embeds, references, reference_weight=reference_weight)
        score.seed = seed
    ...
    if best_image is None or best_score is None:
        raise ValueError("No candidates were provided for reference reranking.")
    return best_image, best_score, scored_candidates


def log_reference_images(references: Iterable[ReferenceImage]) -> None:
    for reference in references:
        print(f"[REFERENCE-RERANK] reference={reference.path} view_label={reference.view_label}")
...
def log_candidate_scores(scores: Iterable[CandidateScore]) -> None:
    for score in scores:
        print(
            ...
            f"reference={score.reference_score:.4f} "
