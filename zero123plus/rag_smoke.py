...
def _adapter_state_dict(checkpoint):
    ...
    for key, value in state_dict.items():
        for prefix in ('rag_adapter.', 'model.rag_adapter.'):
    ...
    if any(key.startswith(('ref_proj.', 'view_embed.')) for key in state_dict):
        return state_dict
    raise ValueError("No RAG adapter weights found in smoke-test checkpoint.")
...
def _save_per_slot_difference_grid(per_slot_report, path):
    ...
    for row, (name, slot_values) in enumerate(comparisons):
        ...
        for slot in range(6):
            ...
            draw.text((x + 8, y + 8), RAG_SLOT_LABELS[slot], fill='black', font=font)
...
def _routing_kwargs(model):
    return {
        'mode': _getattr(model, 'rag_slot_weight_mode', 'local'),
        'sigma_deg': _getattr(model, 'rag_slot_weight_sigma_deg', 80.0),
        'min_weight': _getattr(model, 'rag_slot_weight_min', 0.05),
        'normalize': _getattr(model, 'rag_slot_weight_normalize', True),
        'elevation_weight': _getattr(model, 'rag_slot_weight_elevation_weight', 0.25),
        'cross_view_propagation_enabled': _getattr(model, 'rag_cross_view_propagation_enabled', False),
        'cross_view_propagation_strength': _getattr(model, 'rag_cross_view_propagation_strength', 0.3),
        'cross_view_neighbor_degrees': _getattr(model, 'rag_cross_view_neighbor_degrees', 120.0),
    }
...
def _slot_weight_report(references, slot_weights, model):
    ...
    return {
        'routing_mode': _getattr(model, 'rag_slot_weight_mode', 'local'),
        'sigma_deg': _getattr(model, 'rag_slot_weight_sigma_deg', 80.0),
        'min_weight': _getattr(model, 'rag_slot_weight_min', 0.05),
        'normalize': _getattr(model, 'rag_slot_weight_normalize', True),
        'elevation_weight': _getattr(model, 'rag_slot_weight_elevation_weight', 0.25),
        'global_token_enabled': _getattr(model, 'rag_global_reference_token_enabled', True),
        'global_token_scale': _getattr(
            model,
            'rag_global_reference_token_scale',
            _getattr(model, 'rag_global_scale', 0.05),
        ),
        'cross_view_propagation_enabled': _getattr(model, 'rag_cross_view_propagation_enabled', False),
        'cross_view_propagation_strength': _getattr(model, 'rag_cross_view_propagation_strength', 0.3),
        'cross_view_neighbor_degrees': _getattr(model, 'rag_cross_view_neighbor_degrees', 120.0),
...
def run_post_train_rag_smoke_test(model, checkpoint_path=None):
    """Run a small deterministic RAG ablation using the model already resident on device."""
    output_dir = Path(model.logdir) / 'rag_smoke_test'
    output_dir.mkdir(parents=True, exist_ok=True)
    report_path = output_dir / 'difference_report.json'

    if model.rag_adapter is None:
        raise RuntimeError("RAG smoke test requires an initialized rag_adapter.")
    if not model.rag_smoke_test_cond_image or not model.rag_smoke_test_ref_dir:
        raise ValueError("RAG smoke test requires rag_smoke_test_cond_image and rag_smoke_test_ref_dir.")

    checkpoint_label = 'in_memory_adapter'
    if checkpoint_path:
        checkpoint_path = os.path.abspath(checkpoint_path)
        if not os.path.exists(checkpoint_path):
            raise FileNotFoundError(f"RAG smoke-test checkpoint not found: {checkpoint_path}")
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        incompatible = model.rag_adapter.load_state_dict(_adapter_state_dict(checkpoint), strict=False)
        if incompatible.missing_keys or incompatible.unexpected_keys:
            raise RuntimeError(
                "RAG smoke-test checkpoint is incompatible: "
                f"missing={list(incompatible.missing_keys)}, unexpected={list(incompatible.unexpected_keys)}"
            )
        checkpoint_label = checkpoint_path

    condition_image = Image.open(model.rag_smoke_test_cond_image).convert('RGB')
    references = load_reference_images(
        model.rag_smoke_test_ref_dir,
        metadata_path=model.rag_smoke_test_metadata,
        pose_version='v1.2',
    )
    if not references:
        raise ValueError(f"No smoke-test references found: {model.rag_smoke_test_ref_dir}")
    ...
    shuffled_view_ids = _rotate(view_ids)
    shuffled_slot_weights = _rotate(slot_weights)
    ...
    adapter = model.rag_adapter
    if hasattr(pipeline, 'to'):
        pipeline.to(device)
    adapter.to(device)
    print(f"[RAG-SMOKE] inference device: {device}")
    ...
    common_rag = {
        'rag_adapter': adapter,
        'rag_token_scale': model.rag_token_scale,
        'rag_global_scale': _getattr(
            model,
            'rag_global_reference_token_scale',
            _getattr(model, 'rag_global_scale', 0.05),
        ),
        'rag_global_reference_token_enabled': _getattr(model, 'rag_global_reference_token_enabled', True),
        'rag_match_scale': model.rag_match_scale,
        'rag_near_scale': model.rag_near_scale,
        'rag_nonmatch_scale': model.rag_nonmatch_scale,
        'rag_unknown_scale': model.rag_unknown_scale,
        'rag_spatial_gating': model.rag_spatial_gating,
        'rag_spatial_gate_scale': model.rag_spatial_gate_scale,
        'rag_debug_dump': False,
    }

    conditions = {
        'baseline_no_adapter': {},
        'adapter_no_refs': {'rag_adapter': adapter},
        'adapter_correct_refs_normal_scale': {
            **common_rag,
            'rag_reference_images': reference_images,
            'rag_ref_view_ids': view_ids,
            'rag_ref_slot_weights': slot_weights,
        },
        'adapter_wrong_refs': {
            **common_rag,
            'rag_reference_images': wrong_images,
            'rag_ref_view_ids': view_ids,
            'rag_ref_slot_weights': slot_weights,
        },
        'adapter_shuffled_metadata': {
            **common_rag,
            'rag_reference_images': reference_images,
            'rag_ref_view_ids': shuffled_view_ids,
            'rag_ref_slot_weights': shuffled_slot_weights,
        },
    }
    if model.rag_smoke_test_include_strong_scale:
        multiplier = model.rag_smoke_test_strong_scale_multiplier
        conditions['adapter_correct_refs_strong_scale'] = {
            **conditions['adapter_correct_refs_normal_scale'],
            'rag_token_scale': model.rag_token_scale * multiplier,
            'rag_global_scale': common_rag['rag_global_scale'] * multiplier,
        }
    ...
    try:
        for name, kwargs in conditions.items():
            torch.manual_seed(model.rag_smoke_test_seed)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(model.rag_smoke_test_seed)
            generator = torch.Generator(device=device).manual_seed(model.rag_smoke_test_seed)
            print(f"[RAG-SMOKE] generating {name} ({model.rag_smoke_test_num_inference_steps} steps)")
            output = pipeline(
                condition_image,
                num_inference_steps=model.rag_smoke_test_num_inference_steps,
                generator=generator,
                output_type='pt',
                **kwargs,
            ).images[0]
    ...
    report = {
        ...
        'num_inference_steps': model.rag_smoke_test_num_inference_steps,
        'seed': model.rag_smoke_test_seed,
        'correct_vs_no_refs_mean_abs_diff': _mean_abs_array_diff(
            float_outputs['adapter_correct_refs_normal_scale'], float_outputs['adapter_no_refs']
        ),
        'wrong_vs_correct_mean_abs_diff': _mean_abs_array_diff(
            float_outputs['adapter_wrong_refs'], float_outputs['adapter_correct_refs_normal_scale']
        ),
        'shuffled_vs_correct_mean_abs_diff': _mean_abs_array_diff(
            float_outputs['adapter_shuffled_metadata'], float_outputs['adapter_correct_refs_normal_scale']
        ),
    }
    if 'adapter_correct_refs_strong_scale' in images:
        report['strong_vs_normal_mean_abs_diff'] = _mean_abs_array_diff(
            float_outputs['adapter_correct_refs_strong_scale'],
            float_outputs['adapter_correct_refs_normal_scale'],
        )
    ...
    per_slot_report = {
        ...
        'shuffled_vs_correct': _per_slot_abs_diff(
            float_outputs['adapter_shuffled_metadata'], float_outputs['adapter_correct_refs_normal_scale']
        ),
    }
    ...
    _save_diff_heatmap(
        float_outputs['adapter_shuffled_metadata'],
        float_outputs['adapter_correct_refs_normal_scale'],
        output_dir / 'shuffled_vs_correct_diff_heatmap.png',
    )
    _save_diff_heatmap(
    ...
    threshold = model.rag_smoke_test_difference_threshold
    if report['correct_vs_no_refs_mean_abs_diff'] <= threshold:
        verdict = 'FAIL'
    elif max(report['wrong_vs_correct_mean_abs_diff'], report['shuffled_vs_correct_mean_abs_diff']) <= threshold:
        verdict = 'WARNING'
    else:
        verdict = 'PASS'
        ...
        'objects': [
            {
                ...
                'correct_ref_output': str(output_dir / 'adapter_correct_refs_normal_scale.png'),
                'wrong_ref_output': str(output_dir / 'adapter_wrong_refs.png'),
                'shuffled_metadata_output': str(output_dir / 'adapter_shuffled_metadata.png'),
                ...
                'mean_abs_difference': {
                    ...
                    for key in (
                        ...
                        'shuffled_vs_correct_mean_abs_diff',
                    )
                },
            }
        ],
    ...
    print(f"- shuffled_vs_correct_mean_abs_diff: {report['shuffled_vs_correct_mean_abs_diff']:.8f}")
