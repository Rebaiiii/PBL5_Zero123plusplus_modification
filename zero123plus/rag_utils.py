...
RAG_SLOT_LABELS = ("view_30", "view_90", "view_150", "view_210", "view_270", "view_330", "unknown")
RAG_UNKNOWN_VIEW_ID = 6
...
@dataclass
class RAGReferenceImage:
    image: Image.Image
    path: str
    view_id: int = RAG_UNKNOWN_VIEW_ID
...
def infer_label_from_filename(path: str) -> str:
    ...
    if any(part in ("side", "left", "right", "profile") for part in parts):
        return "side"
    for idx, slot in enumerate(RAG_SLOT_LABELS[:6]):
...
def label_to_view_id(label: str) -> int:
    ...
    if label == "back":
        return 2
    if label in RAG_SLOT_LABELS:
        return RAG_SLOT_LABELS.index(label)
    return RAG_UNKNOWN_VIEW_ID
...
def label_to_slot_weights(label: str) -> List[float]:
    ...
    if label in RAG_SLOT_LABELS[:6]:
        weights = [0.05] * 6
        weights[RAG_SLOT_LABELS.index(label)] = 1.0
        return weights
...
def routed_pose_to_slot_weights(
    ...
    if mode == "local":
        ...
        raise ValueError(f"Unknown rag_slot_weight_mode: {mode}")
...
def load_reference_images(
    ...
    for path in paths:
        ...
        references.append(RAGReferenceImage(
            image=image.copy(),
            path=path,
            view_id=view_id,
            view_label=label,
...
def references_to_pil(references: Iterable[RAGReferenceImage]) -> List[Image.Image]:
    return [reference.image for reference in references]


def references_to_view_ids(references: Sequence[RAGReferenceImage]) -> List[int]:
    return [reference.view_id for reference in references]


def references_to_slot_weights(
    references: Sequence[RAGReferenceImage],
    pose_version: str = "v1.2",
    azimuth_sigma: float = 45.0,
    elevation_sigma: float = 25.0,
    mode: str = "local",
